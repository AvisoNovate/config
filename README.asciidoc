...
The `configure-using` function reads the configuration files and returns the final
configuration map, after reading and merging all the files and apply the configuration
to each component.

The second parameter to `configure-using` is map of options.

`configure-using` generates default profiles from components that declare
a configuration key using `with-config-spec`.
...
[source,clojure]
----
   (config/configure-using {:variants [:overrides]})
----
...
[source,clojure]
----
   (config/configure-using {:additional-files ["overrides/tests.edn"]})
----

Beyond this, it is possible to override how config maps between profile names
and the actual configuration files (see the API documentation for details).
