...
errors about invalid schemas, because there is (for example)
not :web-service top-level key, and no :port key below that.
...
- Instead of a dependency on the configuration service, there is an additional
  field for the port provided in the configuration.
- WebServiceConfig is *just* the configuration of the service
- `with-config-schema` provides an additional key, the top-level key
- The `configure` method gets just the configuration for :web-service; the
  map with the :port key.
